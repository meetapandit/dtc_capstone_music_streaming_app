#!/usr/bin/env bash
# Generates 2026 Q1 eventsim data using Docker (Java 11, amd64)
set -euo pipefail
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting 2026 Q1 generation..."
...
docker run --platform linux/amd64 --rm \
  ...
  eventsim -c "
    ...
    java -XX:+UseG1GC -Xmx4G -jar eventsim-assembly-2.0.jar \
      ...
      --userid 1 \
      --randomseed 1 \
      /output/2026_Q1_control.json
  "

echo "[$(date '+%Y-%m-%d %H:%M:%S')] Done. Output: ${OUTPUT_DIR}/2026_Q1_control.json"
